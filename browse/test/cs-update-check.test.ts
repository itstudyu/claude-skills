/**
 * Tests for bin/cs-update-check bash script.
 *
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'cs-update-check');
...
beforeEach(() => {
  ...
  // Link real cs-config so update_check config check works
  const binDir = join(gstackDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'cs-config'), join(binDir, 'cs-config'));
});
