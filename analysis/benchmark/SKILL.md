...
```bash
eval "$(~/.claude/skills/claude-skills/bin/gstack-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .claude-skills/benchmark-reports
mkdir -p .claude-skills/benchmark-reports/baselines
```
...
Write to `.claude-skills/benchmark-reports/baselines/baseline.json`.
...
Write to `.claude-skills/benchmark-reports/{date}-benchmark.md` and `.claude-skills/benchmark-reports/{date}-benchmark.json`.
