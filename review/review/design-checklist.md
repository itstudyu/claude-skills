...
**Trigger:** Only run this checklist if the diff touches frontend files. Use `cs-diff-scope` to detect:

```bash
source <(~/.claude/skills/gstack/bin/cs-diff-scope <base> 2>/dev/null)
```
