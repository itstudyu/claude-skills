...
function setConfig(key: string, value: string) {
  run(`${BIN}/cs-config set ${key} ${value}`);
}
...
describe('cs-telemetry-log', () => {
  test('appends valid JSONL when tier=anonymous', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill qa --duration 142 --outcome success --session-id test-123`);
  ...
  test('produces no output when tier=off', () => {
    ...
    run(`${BIN}/cs-telemetry-log --skill ship --duration 30 --outcome success --session-id test-456`);
  ...
  test('defaults to off for invalid tier value', () => {
    ...
    run(`${BIN}/cs-telemetry-log --skill ship --duration 30 --outcome success --session-id test-789`);
  ...
  test('includes installation_id for community tier', () => {
    ...
    run(`${BIN}/cs-telemetry-log --skill review --duration 100 --outcome success --session-id comm-123`);
  ...
  test('installation_id is null for anonymous tier', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill qa --duration 50 --outcome success --session-id anon-123`);
  ...
  test('includes error_class when provided', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill browse --duration 10 --outcome error --error-class timeout --session-id err-123`);
  ...
  test('handles missing duration gracefully', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill qa --outcome success --session-id nodur-123`);
  ...
  test('supports event_type flag', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --event-type upgrade_prompted --skill "" --outcome success --session-id up-123`);
  ...
  test('includes local-only fields (_repo_slug, _branch)', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill qa --duration 50 --outcome success --session-id local-123`);
  ...
  test('creates analytics directory if missing', () => {
    ...
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill qa --duration 50 --outcome success --session-id mkdir-123`);
...
describe('.pending marker', () => {
  test('finalizes stale .pending from another session as outcome:unknown', () => {
    ...
    run(`${BIN}/cs-telemetry-log --skill qa --duration 50 --outcome success --session-id new-456`);
  ...
  test('does not finalize own session pending marker', () => {
    ...
    run(`${BIN}/cs-telemetry-log --skill qa --duration 50 --outcome success --session-id same-session`);
  ...
  test('tier=off still clears own session pending', () => {
    ...
    run(`${BIN}/cs-telemetry-log --skill qa --duration 50 --outcome success --session-id off-123`);
...
describe('cs-analytics', () => {
  test('shows "no data" for empty JSONL', () => {
    const output = run(`${BIN}/cs-analytics`);
    expect(output).toContain('no data');
  });

  test('renders usage dashboard with events', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill qa --duration 120 --outcome success --session-id a-1`);
    run(`${BIN}/cs-telemetry-log --skill qa --duration 60 --outcome success --session-id a-2`);
    run(`${BIN}/cs-telemetry-log --skill ship --duration 30 --outcome error --error-class timeout --session-id a-3`);

    const output = run(`${BIN}/cs-analytics all`);
  ...
  test('filters by time window', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill qa --duration 60 --outcome success --session-id t-1`);

    const output7d = run(`${BIN}/cs-analytics 7d`);
    expect(output7d).toContain('/qa');
    expect(output7d).toContain('last 7 days');
  });
});

describe('cs-telemetry-sync', () => {
  test('exits silently with no Supabase URL configured', () => {
    // Default: GSTACK_SUPABASE_URL is not set → exit 0
    const result = run(`${BIN}/cs-telemetry-sync`);
    expect(result).toBe('');
  });

  test('exits silently with no JSONL file', () => {
    const result = run(`${BIN}/cs-telemetry-sync`, { GSTACK_SUPABASE_URL: 'http://localhost:9999' });
    expect(result).toBe('');
  });

  test('does not rename JSONL field names (edge function expects raw names)', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/cs-telemetry-log --skill qa --duration 60 --outcome success --session-id raw-fields-1`);
